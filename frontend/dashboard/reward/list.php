...
if (function_exists('aerp_render_breadcrumb')) {
    aerp_render_breadcrumb([
        ...
        ['label' => 'Quản lý thưởng động']
    ]);
}
?>
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Danh sách thưởng động</h5>
        <a href="<?php echo esc_url(home_url('/aerp-reward-settings/?action=add')); ?>" class="btn btn-primary">
            <i class="fas fa-plus"></i> Thêm mới
        </a>
    </div>
    <div class="card-body">
        <?php // Display messages if any (using Transients API)
        $message = get_transient('aerp_reward_message');
        if ($message) {
            echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
                    ' . esc_html($message) . '
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </div>';
            delete_transient('aerp_reward_message');
        }?>
        <div id="aerp-reward-table-wrapper">
            <?php $table->render(); ?>
        </div>
    </div>
</div>
